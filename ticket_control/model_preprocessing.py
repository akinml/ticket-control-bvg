...
def get_data():
    ...
    data_train[encoder.get_feature_names_out()] = encoder.transform(data_train[["bezirk"]])
    data_test[encoder.get_feature_names_out()] = encoder.transform(data_test[["bezirk"]])
    # drop duplicates
    data_train = data_train.drop_duplicates(subset=["bezirk", "bins"])
    data_test = data_test.drop_duplicates(subset=["bezirk", "bins"])
    # encode_target
    X_train = data_train.drop(columns = ['station_key', 'text', 'station name', 'lines', 'area', 'target', 'bins', 'bezirk', 'month', 'weekday', 'hour'])
    y_train = data_train['target'].map(encode_target)
    X_test = data_test.drop(columns = ['station_key', 'text', 'station name', 'lines', 'area', 'target', 'bins', 'bezirk', 'month', 'weekday', 'hour'])
...
def count_matrix(data):
    count_matrix = data.groupby(["bezirk", "bins"])["station_key"].count().unstack().fillna(0)
